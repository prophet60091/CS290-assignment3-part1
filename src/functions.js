...
bar = function(doubleArray) {
    ...
    var a;
    var i;
    //doubleArray.forEach(function(element) {
    for( i = 0; i < doubleArray.length; i++){
        if (isString(doubleArray[i]) || isNull(doubleArray[i])) {
            return false;
        }else{
...
function parseGit(logArray){
    ...
    var spl = /(^[^\s]+)|"(.*?)"/;
    //var spl = /(^[\d]+)|([A-z0-9]+,\s[\d]\s[\w]+\s[\d]+\s[\d]+:\d+:\d+\s[^\d]+[\d]+)|"(.*?)"/ig
    var logs = [];

    logArray.forEach(function(element, index, array){
        //split it up
        var bande  = element.split(spl);
        //add it to an array
        logs.push(new GitLog(bande[1], new Date(bande[3]), bande[5]));
    });
    return logs;
}
//end your code
